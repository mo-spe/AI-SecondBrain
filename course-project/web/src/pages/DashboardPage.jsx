...
export default function DashboardPage() {
  ...
  const loadItems = () => apiRequest('/knowledge').then(({ items: nextItems }) => setItems(nextItems)).catch(() => {});
  useEffect(() => { loadItems(); }, []);
  ...
  return (
    <div className="page-wrap">
      ...
      <section className="stats-grid">{cards.map(({ label, value, hint, icon: Icon, tone }) => <div className={`stat-card ${tone}`} key={label}><div className="stat-icon"><Icon size={18} /></div><p>{label}</p><strong>{value}</strong><span>{hint}</span></div>)}</section>
