...
public class SearchAPITest {
    ...
    public void testKeywordSearch() {
        ...
        try {
            List<KnowledgeNodeVO> results = knowledgeService.search("Python", 5L, 1L);
    ...
    public void testSemanticSearch() {
        ...
        try {
            List<KnowledgeNodeVO> results = knowledgeService.semanticSearch("Python的特点", 5L, 10, 1L);
            System.out.println("搜索结果数: " + results.size());
            for (int i = 0; i < Math.min(5, results.size()); i++) {
                System.out.println("  " + (i+1) + ". " + results.get(i).getTitle());
            }
    ...
    public void testSemanticSearchWithSpace() {
        ...
        try {
            List<KnowledgeNodeVO> results = knowledgeService.semanticSearch("Python 的特点", 5L, 10, 1L);
            System.out.println("搜索结果数: " + results.size());
            for (int i = 0; i < Math.min(5, results.size()); i++) {
                System.out.println("  " + (i+1) + ". " + results.get(i).getTitle());
            }
